...
def validate_import(payload):
    ...
    if isinstance(raw_employees, dict) and 'employees' in raw_employees:
        employees = raw_employees['employees']
    elif isinstance(raw_employees, dict):
    ...
    if isinstance(history, str):
        ...
        history=list(reader)
        if any(None in row for row in history): raise ValueError('История CSV: лишние значения в строке')
    ...
    eng = engine(); ids=set(eng.employees); newids=set(); skills=set(eng.skills); normalized=[]
    for original in employees:
        e=copy.deepcopy(original)
        if not isinstance(e,dict): raise ValueError('Некорректный профиль')
        for k in ['employee_id','role','grade','skills']:
        ...
        e['full_name']=e.get('full_name') or e['employee_id']
        if not isinstance(e['full_name'],str): raise ValueError('full_name должен быть текстом')
        e.setdefault('department','Не указан'); e.setdefault('work_format','Не указан'); e.setdefault('preferred_language','ru')
        review=e.get('last_review_date')
        if review:
            date.fromisoformat(review)
            if review>TODAY: raise ValueError('Оценка позже даты среза')
        else: e['last_review_date']=None
        hire=e.get('hire_date')
        if hire:
            hired=date.fromisoformat(hire)
            if hire>TODAY: raise ValueError('Дата найма позже даты среза')
        tenure=e.get('tenure_months')
        if tenure is not None and (type(tenure) is not int or tenure<0): raise ValueError('tenure_months: ожидается целое число ≥ 0')
        if tenure is None and hire:
            cutoff=date.fromisoformat(TODAY); e['tenure_months']=max(0,(cutoff.year-hired.year)*12+cutoff.month-hired.month-(cutoff.day<hired.day))
        elif tenure is None: e['tenure_months']=None
    ...
    for r in history:
        ...
        if any(not isinstance(r[k],str) or not r[k].strip() for k in ['record_id','employee_id','event_id','date','status']): raise ValueError('Обязательные поля истории не могут быть пустыми')
        r.update({k:r[k].strip() for k in ['record_id','employee_id','event_id','date','status']})
    ...
    existing={r['record_id']:r for r in DATA['history']}
    for r in history:
        old=existing.get(r['record_id'])
        if old and (old['employee_id'],old['event_id']) != (r['employee_id'],r['event_id']):
            raise ValueError('record_id уже привязан к другой записи')
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            with LOCK:
                if self.path=='/api/complete':
                    ...
                    current=eng.profile(eid,hours)
                    if event['event_id'] not in {item['event_id'] for item in current['recommendations']}:
                        raise ValueError('Рекомендация устарела или активность не входит в топ-3')
                    ...
                    if not e.get('last_review_date') or e['last_review_date']==TODAY: e['skills']=eng.apply(e['skills'],event)
        ...
        except (ValueError,KeyError,TypeError,AttributeError,csv.Error) as e: self.send({'error':str(e)},400)
